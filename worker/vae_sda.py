...
class VideoCLIPVAE(pl.LightningModule):
    def __init__(self, 
                 ...
                 latent_length      : int = 234,
                 ib_dim             : int = 1024,
                 # Training setting
                 lr_warmup_steps        : int = 2000,
                 use_cache_video_feat   : bool = False,
                 on_cavp_feature        : bool = False,
        ...
        self.ib_proj_v = nn.Linear(videoclipvae_config.params.in_channels, ib_dim)
        self.ib_proj_a = nn.Linear(videoclipvae_config.params.in_channels, ib_dim)
        ...
        self.on_cavp_feature = on_cavp_feature
    ...
    def compute_losses(self, v_feat, v_feat_rec, ib_video, ib_audio):
        ...
        if self.image_encoder.visual.proj is not None and not self.on_cavp_feature:
            v_feat_proj = v_feat_proj @ self.image_encoder.visual.proj
            v_feat_rec_proj = v_feat_rec_proj @ self.image_encoder.visual.proj
        else:
            v_feat_proj = nn_func.normalize(v_feat_proj, dim=-1)
            v_feat_rec_proj = nn_func.normalize(v_feat_rec_proj, dim=-1)
        ...
        if self.image_encoder.visual.proj is not None and not self.on_cavp_feature:
            ...
            v_clip_feat_proj = nn_func.normalize(v_clip_feat_proj, dim=-1)
            v_clip_feat_rec_proj = nn_func.normalize(v_clip_feat_rec_proj, dim=-1)
        ...
        if self.image_encoder.visual.proj is not None and not self.on_cavp_feature:
            ...
            v_full_feat_proj = nn_func.normalize(v_full_feat, dim=-1)
            v_full_feat_rec_proj = nn_func.normalize(v_full_feat_rec, dim=-1)
    ...
    def training_step(self, batch, batch_idx, dataloader_idx=0, custom_device=None):
        ...
        # video_feat = nn_func.interpolate(video_feat, size=(self.latent_length,), mode="linear", align_corners=False) 
        video_feat = nn_func.interpolate(video_feat, size=(self.latent_length,), mode="nearest") 
    ...
    def select_random_segments_with_gradients(self, data, segment_length=30, num_segments=3):
        """
        Args:
