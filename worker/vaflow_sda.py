...
class WrappedModel(ModelWrapper):
    def forward(self, x: torch.Tensor, t: torch.Tensor, c: torch.Tensor, w: float = 3.0, **extras):
        t = (t * 1000).long()
        uncond_c = torch.zeros_like(c).to(c.device)
        uncond_pred = self.model(x, t, uncond_c)
        cond_pred = self.model(x, t, c)
...
class VAFlow(pl.LightningModule):
    def __init__(self, 
        ...
        self.cond_proj = nn.Linear(cond_feat_dim, dit_config.params.hidden_size)
    ...
    def training_step(self, batch, batch_idx, dataloader_idx=0, custom_device=None):
        ...
        """ 5. Forward VAFlow. """
        audio_latent_pred = self.vaflow(x=x_t, t=t, c=video_feat_cond)
