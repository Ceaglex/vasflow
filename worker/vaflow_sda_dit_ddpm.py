...
class VAFlow(pl.LightningModule):
    def __init__(self, 
                 ...
                 sample_scheduler_config: Dict = None,
                 # PL training setting 
                 monitor        : str = None, 
        ...
        self.sample_scheduler_config = sample_scheduler_config
    ...
    def training_step(self, batch, batch_idx, dataloader_idx=0, custom_device=None):
        ...
        timesteps = torch.randint(0, self.noise_scheduler.config.num_train_timesteps, (batch_size,), device=audio_latent.device)
        timesteps = timesteps.long()
        t = timesteps.float() / self.noise_scheduler.config.num_train_timesteps
        noisy_latents = self.noise_scheduler.add_noise(audio_latent, noise, timesteps)
            
        """ 4. Sample probability path. """
        if self.noise_scheduler.config.prediction_type == "epsilon":
            target = noise
        elif self.noise_scheduler.config.prediction_type == "v_prediction":
            target = self.noise_scheduler.get_velocity(audio_latent, noise, timesteps)
        else:
        ...
        """ 6. Compute loss. """
        loss = nn_func.mse_loss(audio_latent_pred, target, reduction="mean")
    ...
    def on_validation_epoch_start(self):
        ...
        self.sample_scheduler = instantiate_from_config(self.sample_scheduler_config)
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0, custom_device=None):
        ...
        for _rand_i in range(self.num_samples_per_prompt):
            ...
            audio_latent = randn_tensor((batch_size, self.latent_in_dim, self.latent_length), device=device, generator=generator)

            # 1.2 Rotary embedding.
            rotary_embedding = get_1d_rotary_pos_embed(
                self.rotary_embed_dim,
                audio_latent.shape[2] + 1,
                use_real=True,
                repeat_interleave_real=False,
            )

            # 2. Sample.
            self.sample_scheduler.set_timesteps(self.sample_steps, device=device)
            timesteps = self.sample_scheduler.timesteps
            for i, t in enumerate(timesteps):
                audio_latent_model_input = torch.cat([audio_latent] * 2) if do_classifier_free_guidance else audio_latent
                audio_latent_model_input = self.sample_scheduler.scale_model_input(audio_latent_model_input, t)
                video_feat_cond_input = torch.cat([video_feat_cond] * 2) if do_classifier_free_guidance else video_feat_cond
                
                t_in = t.unsqueeze(0).repeat(audio_latent_model_input.shape[0]).to(audio_latent_model_input)
                t_in = t_in.float() / self.sample_scheduler.config.num_train_timesteps
                audio_latent_pred = self.vaflow(
                    audio_latent_model_input, 
                    t_in, 
                    encoder_hidden_states=video_feat_cond_input,
                    global_hidden_states=None,              # Removed in this version.
                    rotary_embedding=rotary_embedding,
                ).sample
                
                if do_classifier_free_guidance:
                    audio_latent_pred_uncond, audio_latent_pred_cond = audio_latent_pred.chunk(2)
                    audio_noise_pred = audio_latent_pred_uncond + self.guidance_scale * (audio_latent_pred_cond - audio_latent_pred_uncond)
                    
                audio_latent = self.sample_scheduler.step(audio_noise_pred, t, audio_latent, ).prev_sample
