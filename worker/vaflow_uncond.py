...
class VAFlow(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx, dataloader_idx=0, custom_device=None):
        ...
        with torch.no_grad():
            ...
            audio_latent = self.audio_codec.encode(waveform).latent_dist    # [B, 64, 234]
            audio_latent = audio_latent.sample()                            # [B, 64, 234]
        ...
        # NOTE
        video_latent = torch.randn_like(audio_latent).to(device)
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0, custom_device=None):
        ...
        # NOTE
        video_latent = torch.rand((batch_size, 234, 64)).to(device)
