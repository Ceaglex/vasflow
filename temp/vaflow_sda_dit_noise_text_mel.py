...
class VAFlow(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx, dataloader_idx=0, custom_device=None):
        ...
        audio_latent = torch.cat([audio_latent, expanded_phone_latent], dim=1)      # [bs, original_channel + phone_latent_dim + 1, latent_length]
        video_latent = torch.cat([video_latent, expanded_phone_latent], dim=1)      # [bs, original_channel + phone_latent_dim + 1, latent_length]
    ...
    def validation_step_(self, batch, batch_idx, dataloader_idx=0, custom_device=None):
        ...
        for _rand_i in range(self.num_samples_per_prompt):
            ...
            latent_uncond = expanded_phone_latent_uncond
            # latent_uncond = expanded_phone_latent
